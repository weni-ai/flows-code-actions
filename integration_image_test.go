...
func (suite *ImageIntegrationTestSuite) TestCreateCodeWithHeader() {
	complexCode := `
import json
import datetime

def Run(engine):
    content_type = engine.header.get('Content-Type')
...
`
	...
	q := req.URL.Query()
	q.Add("project_uuid", suite.projectUUID)
	q.Add("name", "Header Integration Test")
	q.Add("type", "endpoint")
	q.Add("language", "python")
	req.URL.RawQuery = q.Encode()
	...
	suite.Assert().Equal("application/json", result["content_type"])
